...
class WebSocketClient extends EventEmitter {
  ...
  constructor(host, port, options = {}) {
    ...
    this.autoVoiceCloning = options.autoVoiceCloning !== undefined ? options.autoVoiceCloning : false;
  ...
  _handleMessage(event) {
    ...
    try {
      ...
      // Debug: Log all messages to see what the new server sends
      ...
      if (!this.isServerReady && data.message === "SERVER_READY") {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const url = this.port ? `${protocol}://${this.host}:${this.port}` : `${protocol}://${this.host}`;
        ...
        this.socket.onopen = () => {
          ...
          if (this.sourceLanguage !== null && this.sourceLanguage !== undefined) {
            ...
            // Also send as 'language' for backward compatibility
            options.language = this.sourceLanguage;
          }
          ...
          if (this.saveToDashboard !== null && this.saveToDashboard !== undefined) {
            ...
            // Also send as save_after for server compatibility
            options.save_after = this.saveToDashboard;
          }
          ...
          if (this.autoVoiceCloning !== null && this.autoVoiceCloning !== undefined) {
            options.autoVoiceCloning = this.autoVoiceCloning;
          }
  ...
  sendAudio(audioData) {
    if (this.socket && this.socket.readyState === WebSocket.OPEN && this.isServerReady) {
      ...
      // Audio is flowing correctly - no need to log every packet
    } else {
      const socketState = this.socket ? 
        `readyState=${this.socket.readyState} (CONNECTING=0, OPEN=1, CLOSING=2, CLOSED=3)` : 
        'socket is null';
      console.warn(`[WebSocket] Cannot send audio - ${socketState}, Server ready: ${this.isServerReady}`);
    }
  }
